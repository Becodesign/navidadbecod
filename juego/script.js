...
const updateScoreDisplay = () => {
   document.getElementById('scoreValue').textContent = gameConfig.score;
   document.getElementById('levelValue').textContent = gameConfig.level;
};

const updateFinalScore = () => {
   document.getElementById('finalScoreValue').textContent = gameConfig.score;
   document.getElementById('finalLevelValue').textContent = gameConfig.level;
};
...
const addScore = (points) => {
   ...
   // Check for level up
   const newLevel = Math.floor(gameConfig.score / 1000) + 1;
   if (newLevel > gameConfig.level) {
      gameConfig.level = newLevel;
      updateScoreDisplay();

      // Spawn additional asteroids for new level
      const additionalAsteroids = 2;
...
const render = (timeStamp) => {
   ...
   // Only update objects if game is playing
...
class Asteroid extends Entity {
   ...
   explode() {
      ...
      const points = Math.floor(this.radius / 10) * 10;
      addScore(points);
   }
...
class Player extends Entity {
   ...
   checkAsteroidCollision() {
      objectBuffer
         ...
         .find((asteroid) => {
            const distance = v3mag(v3sub(this.position, asteroid.position));
            const playerRadius = 20; // Approximate player radius
